...
| relay SQLite xShmMap crash on GKE SSD PVC | SQLite WAL maps shared memory (xShmMap) incompatible with network-attached block storage; PostgreSQL manages its own shared memory — no issue | Replace rusqlite with sqlx+postgres; dedicated rsky-relay-postgres StatefulSet (isolated from postgresql-0). LISTEN/NOTIFY for instant ban refresh. Hostname: rsky-relay-postgres:5432 | relay-c001..c005 |
| CloudNativePG Pooler CRD annotation too large for kubectl apply | poolers.postgresql.cnpg.io CRD annotation exceeds 262144 byte limit | Use `--server-side --force-conflicts` flag on kubectl apply; or skip CloudNativePG entirely (plain postgres StatefulSet is simpler and supports LISTEN/NOTIFY natively) | 2026-05-07 |
| RELAY_PG_USER / RELAY_PG_PASSWORD missing from deploy secrets | envsubst in deploy.yml fails silently if secrets not set in GitHub — literals ${RELAY_PG_USER} written to cluster | Set secrets: `gh secret set RELAY_PG_USER` and `gh secret set RELAY_PG_PASSWORD` before pushing relay code | 2026-05-07 |
| relay CrashLoop after postgres migration — "no tokio runtime in get_cursor" | crawler Manager runs on plain OS thread with no Tokio context; `block_in_place + Handle::try_current()` requires a Tokio-managed thread | Add `rt: tokio::runtime::Runtime` (new_current_thread) to Manager; use `self.rt.block_on(async {...})` in get_cursor and refresh_bans. File: rsky-relay/src/crawler/manager.rs | 2026-05-07 |
| [lints] workspace = true in relay Cargo.toml breaks cargo check | Prior agent added `[lints] workspace = true` to rsky-relay/Cargo.toml but the `[workspace.lints]` section was never committed to root Cargo.toml | Remove `[lints] workspace = true` from rsky-relay/Cargo.toml | 2026-05-07 |
| relay EADDRNOTAVAIL despite postgres reachable — StatefulSet stripped of all config | kubectl patch on StatefulSet replaced entire pod template spec, stripping envFrom/command/args/ports/probes/resources/volumeMounts; relay ran old image with no DATABASE_URL at all | ALWAYS apply from file: `sed 's|IMAGE_TAG|<sha>|g' k8s/rsky-relay/statefulset.yaml > /tmp/sts.yaml && kubectl apply -f /tmp/sts.yaml`. NEVER kubectl patch a StatefulSet pod template without the full spec | 2026-05-07 |
| relay validator "pool timed out" — PgPool max_connections=10 insufficient | Validator, CrawlerManager RT, PgListener, Server, and Resolver PLC export all share one pool; 10 connections exhausted under load | Increase max_connections 10→20 in rsky-relay/src/main.rs PgPoolOptions | 2026-05-07 |
| relay server listHosts/requestCrawl "no tokio runtime in server block_on_db" | Server runs on blocking OS thread (thread::scope); block_on_db() used block_in_place+Handle::try_current() which requires Tokio context | Add `rt: tokio::runtime::Runtime` to Server struct; change block_on_db() to take &rt; update all 8 call sites. File: rsky-relay/src/server/server.rs | 2026-05-07 |
| feedgen CrashLoopBackOff — pgbouncer rsky_feedgen pool exhausted on startup | 3 pools × pool_size=5 = 15 connections; pgbouncer pool_size=20 saturated by stale connections from previous crash restarts (server_idle_timeout=300s) | Reduce feedgen pool_size 5→3; increase pgbouncer pool_size 20→40; reduce server_idle_timeout 300→60s | 2026-05-07 |
| relay validator panics in Drop — "Cannot start a runtime from within a runtime" | Drop called from inside tokio::spawn task context; Builder::new_current_thread().build().block_on() panics when nested in existing runtime | Use block_in_place + Handle::current().block_on() when inside Tokio context; fall back to new runtime only when Handle::try_current() returns Err. File: rsky-relay/src/validator/manager.rs Drop impl | 2026-05-07 |
| appview getAuthorFeed returns empty feed when called with handle | get_author_feed DB query WHERE p.creator = $1 requires DID; handler passed raw actor param which could be handle | Resolve actor via get_profile() (accepts handle OR DID) before querying posts; use resolved DID. File: rsky-appview/crates/appview-api/src/feed.rs | 2026-05-07 |
| pgbouncer transaction mode breaks Diesel (PDS + feedgen) | Diesel uses named prepared statements (connection-scoped); pgbouncer transaction mode routes each tx to a different backend conn — named stmt not found | NEVER use pgbouncer transaction mode with Diesel. Keep pool_mode=session. sqlx (appview) works in either mode with statement_cache_capacity(0). File: k8s/pgbouncer/configmap.yaml | 2026-05-07 |
| appview dual pool exhausts pgbouncer session pool | appview-bin/main.rs created pool(32) AND AppStateInner::new() created a second pool (~10 default) — 42 conns > pool_size=40 → 504s and apparent CORS errors | Add AppStateInner::new_with_pool(PgPool) constructor; share single pool from main.rs. Reduce main pool to 15. File: appview-api/src/lib.rs, appview-bin/src/main.rs | 2026-05-07 |
| feedgen DB empty — migrations never applied | rsky-feedgen Dockerfile doesn't run migrations at startup; Diesel embed_migrations!() not called in main | Apply migrations manually: concatenate all up.sql files and run via kubectl exec postgresql-0 -- psql -U postgres -d rsky_feedgen -f /tmp/feedgen_all_migrations.sql | 2026-05-07 |
| relay requestCrawl returns 400 — body not read after peek() | Server uses peek() to inspect HTTP headers without consuming bytes; POST body in later TCP segments not captured by peek buf | After peek, read Content-Length header, call tcp.read_exact() for the remainder. File: rsky-relay/src/server/server.rs | 2026-05-07 |
| subscribeFeed WebSocket 404 causing React Error #419 in web client | tools.knowme.feed.subscribeFeed route not registered in appview router; client attempted WSS connection, got 404 | Add 501 stub route returning JSON error; prevents React crash. File: rsky-appview/crates/appview-api/src/lib.rs | 2026-05-07 |
